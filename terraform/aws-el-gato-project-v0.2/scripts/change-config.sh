...
mysql_dump_creation_script_path="/tasks/terraform/aws-el-gato-project-v0.2/docker/LAMP/wordpress/mysql-dump.sh"
...
# Changes variables in change-wp-config.sh
...
# Changes variables in mysql-dump.sh
sed -i "s|db_name=.*$|db_name=${db_name}|" ${mysql_dump_creation_script_path}
sed -i "s|db_user=.*$|db_user=${db_user}|" ${mysql_dump_creation_script_path}
sed -i "s|db_host=.*$|db_host=${db_host}|" ${mysql_dump_creation_script_path}
sed -i "s|db_pass=.*$|db_pass=${db_pass}|" ${mysql_dump_creation_script_path}
